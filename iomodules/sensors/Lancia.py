...
# Originally forked from John Snowdens Cowsworth ECU tool https://github.com/megatron-uk/PyLancia
# Copyright (C) 2018  John Snowdon
# Made to work with a Lancia Delta with the IAW 049 ECU
# 
...
class LanciaSensors():
	...
	def __init__(self, ecuType = None, pressureType = None):
		...
		logger.info("Starting Lancia ECU sensor module")
		...
		self.supportedECU = ["L8"]
		self.ecuType = False
		
		# Comms parameters
		self.comms = {
			'device' : settings.Lancia_ECU_USB,
			'baud' : "7812",
			'bits' : 8,
			'parity' : "N",
			'stopbits' : 1,
	...
	def __translate__(self, sensorId, rawValue):
		""" Translate a raw value from a Lancia sensor into a real-world number """
	...
	def __connectECU__(self):
		...
		try:
			...
			self.connected = False
			self.serial = False
			logger.fatal("Unable to open requested serial port for Lancia ECU module")
			logger.fatal("%s" % e)
			logger.fatal("")
	...
	def __disconnectECU__(self):
		...
		try:
			self.serial.close()
			logger.info("Closed serial port for Lancia ECU module")
			self.serial = False
		except Exception as e:
			logger.warn("Unable to close serial port for Lancia ECU module")
			logger.warn("%s" % e)
